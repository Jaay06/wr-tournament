...
export function SettingsForm({
  ...
  const [values, setValues] = useState({
    name,
    region,
    deadlineLocal: registrationDeadline,
    inviteEnabled,
    teamRegistrationEnabled,
  });
  const [state, formAction, pending] = useActionState<SettingsState, FormData>(
    async (previous, data) => {
      try {
        return await updateTournamentSettings(previous, data);
      } catch {
        return {
          error:
  ...
  return (
    ...
    >
      <FieldGroup className='grid gap-4 tablet:grid-cols-2'>
        ...
        <Field>
          ...
          <Input
            ...
            onChange={(event) =>
              setValues({ ...values, deadlineLocal: event.target.value })
            }
            disabled={pending}
            id='deadlineLocal'
            name='deadlineLocal'
          ...
          <FieldDescription className='text-xs'>
            Leave this blank to keep registration open. You can extend or clear
            it later.
          </FieldDescription>
        </Field>
      </FieldGroup>
      <input
        defaultValue={
          registrationDeadline ? `${registrationDeadline}:00.000Z` : ''
        }
      ...
      <Field
        className='rounded-md border border-border bg-secondary px-3.5 py-3 text-foreground'
      ...
      >
        ...
        <Switch
          ...
          onCheckedChange={(checked) =>
            setValues({ ...values, inviteEnabled: checked })
          }
          disabled={pending}
          id='inviteEnabled'
          name='inviteEnabled'
      ...
      >
        <FieldContent>
          <FieldLabel className='font-semibold' htmlFor='teamRegistrationEnabled'>
            Accept team submissions
          </FieldLabel>
          <FieldDescription className='text-xs'>
            When off, players cannot create new teams or submit rosters.
          </FieldDescription>
        </FieldContent>
        <Switch
          checked={values.teamRegistrationEnabled}
          onCheckedChange={(checked) =>
            setValues({ ...values, teamRegistrationEnabled: checked })
          }
          disabled={pending}
...
export function InviteCodeForm() {
  ...
  async function copyCode() {
    ...
    try {
      ...
      setCopyState('Copy failed. Select and copy the code above.');
    }
  }
    ...
    <div className='flex flex-col gap-4'>
      <AlertDialog onOpenChange={setConfirmOpen} open={confirmOpen}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Replace the current invite code?
            </AlertDialogTitle>
            <AlertDialogDescription>
              Any link or code already shared will stop working immediately.
              Existing participants keep their access.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      {state.code ? (
        <Alert
          aria-live='polite'
          className='rounded-md border-primary/30 bg-primary/10 text-foreground'
        >
          ...
          <p className='mt-2 mb-0 text-xs leading-5 text-secondary-foreground'>
            Copy it now. It will not be shown again, and generating another code
      ...
      <div className='flex items-start gap-2 rounded-lg border border-warning/30 bg-warning-soft p-3 text-xs leading-relaxed text-warning'>
        ...
        <p className='m-0'>
          A new code immediately invalidates every invite link already shared.
        </p>
      </div>
    </div>
  );
