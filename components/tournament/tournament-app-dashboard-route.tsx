      ...
      <div className="grid items-start gap-[18px] desktop:grid-cols-[minmax(0,1fr)_280px]">
        <Card className="min-w-0 rounded-[14px] p-[18px] ring-0">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0"><Kicker>YOUR TEAM</Kicker><div className="mt-2 flex flex-wrap items-center gap-2"><h2 className="m-0 truncate text-2xl font-bold">{hasTeam ? liveTeam?.name ?? 'Void Hunters' : 'No team yet'}</h2>{hasTeam && <span className="font-mono text-[9px] text-muted-foreground">{liveTeam?.status === 'submitted' ? 'SUBMITTED' : 'DRAFT'}</span>}</div><p className="mt-1 mb-0 text-sm text-muted-foreground">{hasTeam ? 'Captain ' + captainName + ' - ' + (teamMembers.length || 6) + ' members' : teamRegistrationEnabled ? 'Create a team or browse draft rosters.' : 'Team creation is currently paused. Browse existing rosters.'}</p></div>
